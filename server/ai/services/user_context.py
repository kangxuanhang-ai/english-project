...
# progress 工具返回的已掌握单词上限（防止超大 payload）
MAX_MASTERED_WORDS_IN_PROGRESS = 500


async def fetch_user_progress_data(user_id: str) -> dict | None:
    """查询用户学习进度（实时读库）。"""
    async with async_session() as session:
        ...
        mastered_words_result = await session.execute(
...
async def fetch_user_progress_snapshot(user_id: str) -> str:
    data = await fetch_user_progress_data(user_id)
    if not data:
        return ""
